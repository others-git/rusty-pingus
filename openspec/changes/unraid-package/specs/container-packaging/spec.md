...
The project SHALL provide a `Dockerfile` at the repo root that compiles rusty-pingus via a Rust builder stage and produces a minimal Debian-based runtime image.

#### Scenario: Builder stage compiles the binary
- **WHEN** `docker build` is run in the repo root
- **THEN** the builder stage uses a `rust:*-slim-bookworm` image to run `cargo build --release`

#### Scenario: Runtime stage is minimal
- **WHEN** the image is built
- **THEN** the runtime stage is based on `debian:bookworm-slim` and only includes `ca-certificates`, `libssl3`, and the compiled binary

#### Scenario: Non-root execution
- **WHEN** the container is started
- **THEN** the binary runs as a non-root user (e.g., `appuser`) created during the image build
...
### Requirement: Entrypoint script maps env vars to CLI flags
The image SHALL include an `entrypoint.sh` script that translates Docker environment variables into the binary's CLI arguments before exec'ing the binary.

#### Scenario: Env vars translated to CLI flags
- **WHEN** the entrypoint script runs
- **THEN** it constructs `--bind`, `--config`, `--monitors`, and `--db` arguments from `BIND_PORT` and the fixed mount paths, then exec's the binary
