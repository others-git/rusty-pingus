# Traceroute Monitor Spec

## Purpose

Defines the `traceroute` monitor type, which traces the per-hop ICMP path to a target — recording each hop's reachability and round-trip time (with per-run min/avg/max) — and exposes that data aggregated over a requested time range.
