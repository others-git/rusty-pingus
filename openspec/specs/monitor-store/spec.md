...
Defines the runtime monitor store: a thread-safe in-memory source of truth for monitor definitions, loaded from and persisted to `monitors.toml`, including migration of legacy `[[monitors]]` entries out of `config.toml`.
