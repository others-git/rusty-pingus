...
Defines the `border` monitor type, which localizes connectivity faults by ICMP-probing the local gateway and an upstream reference each cycle and classifying the outcome (OK, ISP/upstream down, or LAN/gateway down).
