...
use crate::config::TcpMonitorConfig;
...
mod tests {
    ...
    fn cfg(host: &str, port: u16, timeout: u64) -> TcpMonitorConfig {
        TcpMonitorConfig { name: "test".into(), host: host.into(), port, interval_secs: 60, timeout_secs: timeout }
    }

    #[tokio::test]
    async fn tcp_loopback_refused() {
        // Port 1 on loopback should be refused instantly
        let result = run(&cfg("127.0.0.1", 1, 3)).await;
